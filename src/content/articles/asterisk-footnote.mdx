...
Sometimes the design calls for footnotes indicated by an asterisk (<span aria-hidden="true">\*</span>), and while providing a two-way connection (with jump-links) between the footnote reference and the footnote itself might be the best solution for everyone, it is not always possible because, as the <abbr title="Product Manager">PM</abbr> says:
...
However, I soon discovered that `aria-details` is <Link href="https://a11ysupport.io/tech/aria/aria-details_attribute#support-table-by-assertion-and-at-sr">not widely supported yet</Link> and (as I tested it) also needs to be applied to an interactive element to work properly (the latter constraint also applies to `aria-describedby`).
