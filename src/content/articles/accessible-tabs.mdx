...
<aside class="callout" aria-label="Note about inline conditionals">
  The inline conditionals and template literals are used for the sake of brevity. They illustrate how the initial state would be derived
  dynamically, but the resulting markup is equivalent to static HTML with fixed values.
</aside>
...
This makes the intent clearer, simplifies JavaScript interaction, and keeps the state declarative rather than relying on imperative style changes, closely following the <Link href="https://en.wikipedia.org/wiki/Rule_of_least_power">Rule of Least Power</Link>.
